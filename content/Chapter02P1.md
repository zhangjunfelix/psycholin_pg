+++
title = "Chapter 2 Part 1: Speech Planning & Errors I"
...
[Link to Chapter 2](/psycholin_pg/pdfs/C2.pdf)   
    - speech production
    - speech errors
    - Tip-of-the-tongue experiences
    - Picture-naming and picture-word interference studies

[Levelt_1999](/psycholin_pg/pdfs/Levelt_1999.pdf)
...
- **Myth vs. Reality**:  
  ...
  - ✅ Reality: Complex hidden processes connect "idea" to physical speech (focus of this chapter).  
...
- **What is a Lemma?**  
  Intermediate mental representation with:  
  1. **Semantic info**: Word meaning (e.g., "cat" = small furry mammal).  
  2. **Syntactic info**: How to use it grammatically (e.g., "cat" = noun; fits in "The ____ sat").  
...
1. **Activate Phonemes**: Retrieve speech sounds (e.g., *cat* → /k/, /æ/, /t/).  
2. **Syllabification**: Organize phonemes into syllables (speech is produced in syllables, not isolated sounds):  
...
- **TOT Examples**:  
  1. Scrooge’s first name → *Ebenezer*.  
  2. Small Far East boat → *sampan*.  
  3. Whale secretion for perfume → *ambergris*.  
  4. Stamp collector → *philatelist*.  
...
- **Conditions**:  
  - ✅ Identity: "lemon" over a lemon → faster (shared lemma).  
...
1. **Lexical Bias Effect**: Sound errors mostly make real words (e.g., "blip" → "slip" [real] vs. "tlip" [nonword])—feedback from phonology to lemmas boosts real words.  
2. **Mixed Errors**: Errors are semantically + phonetically similar (e.g., "oyster" → "lobster" [seafood + similar sound])—cascading activation boosts overlapping words.  
...
- **Definition**: Blueprint for articulation—specifies:  
  1. Which articulators to use (e.g., lips for /p/).  
  2. Constriction location (e.g., alveolar ridge for /t/).  
  3. Constriction degree (e.g., full blockage for /p/).  
  4. Movement dynamics (e.g., fast lip release for /p/).  
...
1. **Prosodic Disruption**: Abnormal pitch, accent, or pausing.  
2. **Syllabification Disruption**: Syllables spoken in isolation (e.g., "Ess-cort-us" instead of "Ess-core-tuss").  
3. **Articulation Errors**: Wrong phoneme location (back → front) or missing phonemes (e.g., *spl* → *sl*).  
